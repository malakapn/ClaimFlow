...
async function generatePDF(data: ClaimData, tier: Tier) {
  ...
  doc.setFontSize(10);
  doc.setTextColor(20, 20, 20);
  const narrative = `${data.plName} brings this small claims action against ${data.defName} arising from ${CLAIM_TYPES.find(c => c.id === data.claimType)?.label?.toLowerCase() || "a dispute"} on ${data.incidentDate}. The incident resulted in damages totaling $${data.amountTotal}. ${data.incidentDesc}`;Desc}\n\nI have evidence of this claim, including:\n${data.evidence || "Documentation available upon request"}\n\nI respectfully request judgment in the amount of $${data.amountTotal} plus court costs. Thank you.`;
